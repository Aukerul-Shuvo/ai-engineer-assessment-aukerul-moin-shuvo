...
``search_documents`` returns a compact, model-facing view of a hybrid search: the passage text,
...
from app.retrieval.store import DocumentSummary, HybridStore, RetrievalMode
...
async def search_documents(
    store: HybridStore, query: str, k: int = 5
...
async def list_documents(store: HybridStore) -> DocumentCatalog:
